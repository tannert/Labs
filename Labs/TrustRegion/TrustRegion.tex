...
\end{algorithm}
\begin{problem}
Implement the trust-region algorithm using the following function declaration.  Note that you don't have to actually solve (\ref{eq:step}) for $p_k$; you'll worry about that in Problem 2. In the mean time, you have a parameter \li{subprob} which you can assume does that for you.
\begin{lstlisting}
